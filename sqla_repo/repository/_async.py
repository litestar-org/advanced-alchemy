...
from sqla_repo.repository._util import get_instrumented_attr, wrap_sqlalchemy_exception
from sqla_repo.repository.abc import AbstractAsyncRepository
...
class SQLAlchemyAsyncRepository(AbstractAsyncRepository[ModelT], Generic[ModelT]):
    ...
    async def upsert_many(
        ...
        Args:
            data: Instance to update existing, or be created. Identifier used to determine if an
                existing instance exists is the value of an attribute on ``data`` named as value of
                :attr:`~sqla_repo.abc.AbstractAsyncRepository.id_attribute`.
            attribute_names: an iterable of attribute names to pass into the ``update`` method.
