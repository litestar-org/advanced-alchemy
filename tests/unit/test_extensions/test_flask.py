...
class TestFlaskExtension:
    """Test cases for the Flask extension."""

    @pytest.fixture(scope="function", autouse=True)
    def setup_database(self, tmp_path: Path) -> Generator[Path, None, None]:
    ...
    def test_sync_extension_init(self, setup_database: Path) -> None:
        app = Flask(__name__)
    ...
    def test_sync_extension_init_with_app(self, setup_database: Path) -> None:
        app = Flask(__name__)
    ...
    def test_sync_extension_multiple_init(self, setup_database: Path) -> None:
        app = Flask(__name__)
    ...
    @pytest.mark.asyncio
    async def test_async_extension_init(self, setup_database: Path) -> None:
        app = Flask(__name__)
    ...
    @pytest.mark.asyncio
    async def test_async_extension_init_single_config_no_bind_key(self, setup_database: Path) -> None:
        app = Flask(__name__)
    ...
    @pytest.mark.asyncio
    async def test_async_extension_init_with_app(self, setup_database: Path) -> None:
        app = Flask(__name__)
    ...
    @pytest.mark.asyncio
    async def test_async_extension_multiple_init(self, setup_database: Path) -> None:
        app = Flask(__name__)
    ...
    @pytest.mark.asyncio
    async def test_sync_and_async_extension_init(self, setup_database: Path) -> None:
        app = Flask(__name__)
    ...
    def test_multiple_binds(self, setup_database: Path) -> None:
        app = Flask(__name__)
    ...
    @pytest.mark.asyncio
    async def test_multiple_binds_async(self, setup_database: Path) -> None:
        app = Flask(__name__)
    ...
    @pytest.mark.asyncio
    async def test_mixed_binds(self, setup_database: Path) -> None:
        app = Flask(__name__)
    ...
    def test_sync_autocommit(self, setup_database: Path) -> None:
        app = Flask(__name__)
    ...
    def test_sync_autocommit_with_redirect(self, setup_database: Path) -> None:
        app = Flask(__name__)
    ...
    def test_sync_no_autocommit_on_error(self, setup_database: Path) -> None:
        app = Flask(__name__)
    ...
    @pytest.mark.asyncio
    async def test_async_autocommit(self, setup_database: Path) -> None:
        app = Flask(__name__)
        ...
        with app.test_client() as client:
            ...
            result = extension.portal_provider.portal.call(
                session.execute, select(User).where(User.name == "test_async")
            )
            assert result.scalar_one().name == "test_async"

    @pytest.mark.asyncio
    async def test_async_autocommit_with_redirect(self, setup_database: Path) -> None:
        app = Flask(__name__)
    ...
    @pytest.mark.asyncio
    async def test_async_no_autocommit_on_error(self, setup_database: Path) -> None:
        app = Flask(__name__)
    ...
    @pytest.mark.asyncio
    async def test_async_portal_cleanup(self, setup_database: Path) -> None:
        app = Flask(__name__)
    ...
    @pytest.mark.asyncio
    async def test_async_portal_explicit_stop(self, setup_database: Path) -> None:
        app = Flask(__name__)
    ...
    @pytest.mark.asyncio
    async def test_async_portal_explicit_stop_with_commit(self, setup_database: Path) -> None:
        app = Flask(__name__)
            ...
            async def get_user() -> User | None:
                async with session:
                    result = await session.execute(
                        select(User).where(User.name == "test_async_explicit_stop_with_commit")
                    )
                    return result.scalar_one_or_none()
