...
from sqlalchemy.orm import Mapped, mapped_column, DeclarativeBase, Mapper
...
from advanced_alchemy.repository import SQLAlchemySyncRepository, SQLAlchemyAsyncRepository
...
def test_create_async_service_dependencies( ) -> None:
    ...
    with patch("advanced_alchemy.extensions.litestar.di.create_service_provider") as mock_create_provider:
...
def test_create_service_dependencies_with_filters( ) -> None:
    ...
    with patch("advanced_alchemy.extensions.litestar.di.create_service_provider") as mock_create_provider:
        with patch("advanced_alchemy.extensions.litestar.di.create_filter_dependencies") as mock_create_filters:
...
def test_create_filter_dependencies_cache_miss( ) -> None:
    ...
    with patch.object(dep_cache, "get_dependencies", return_value=None) as mock_get:
        with patch.object(dep_cache, "add_dependencies") as mock_add:
            with patch("advanced_alchemy.extensions.litestar.di._create_statement_filters", return_value=mock_deps) as mock_create:
                config = cast(FilterConfig, {"key1": 1, "key2": 2})
                deps = create_filter_dependencies(config)
