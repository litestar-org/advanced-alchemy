...
def test_delete_with_auto_expunge_and_commit_sync(session: Session) -> None:
    ...
    # Setup: Create a user
    repo = UserRepositorySync(session=session)
    ...
    # Verify: Object is detached from session
    assert is_object_detached(deleted), "Deleted object should be detached"
    assert not is_object_in_session(deleted, session), "Deleted object should not be in session"


def test_delete_where_with_auto_expunge_and_commit_sync(session: Session) -> None:
    ...
    # Setup: Create users
    repo = UserRepositorySync(session=session, auto_expunge=True, auto_commit=True)
    repo.add_many(
        [
            User(name="Alice", active=True),
            User(name="Bob", active=False),
    ...
    for obj in deleted:
        assert is_object_detached(obj), "Deleted object should be detached"
        assert not is_object_in_session(obj, session), "Deleted object should not be in session"
...
@pytest.mark.sqlite
def test_sqlite_delete_with_auto_expunge(session: Session) -> None:
    """SQLite-specific test for delete with auto_expunge."""
    test_delete_with_auto_expunge_and_commit_sync(session)
