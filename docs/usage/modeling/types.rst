...
GUID on Oracle 23ai and UUIDv7 Ordering
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Advanced Alchemy's :class:`~advanced_alchemy.types.GUID` stores UUIDs as
``RAW(16)`` on Oracle. Oracle 23ai does **not** add a native ``UUID`` type;
``RAW(16)`` remains the canonical storage and ``GUID`` requires no changes
to benefit from 23ai. UUIDs are generated by the application (not via
``SYS_GUID()``) so that values are stable across inserts and can be assigned
deterministically by client code, ORM defaults, or test fixtures.

For primary keys, prefer **UUIDv7** (time-ordered, RFC 9562) over UUIDv4
(random). UUIDv7's leading-timestamp bytes give monotonic insert order,
which keeps Oracle's B-tree indexes balanced and reduces hot-leaf
contention. UUIDv4's random distribution causes index fragmentation and
extra I/O on high-volume inserts.

The optional ``uuid-utils`` dependency provides ``uuid7()``, and
Advanced Alchemy ships ready-to-use bases that wire it into the primary
key automatically:
...
See :class:`~advanced_alchemy.base.UUIDv7Base` and
:class:`~advanced_alchemy.base.UUIDv7AuditBase` for the time-ordered
variants, or :class:`~advanced_alchemy.mixins.UUIDv7PrimaryKey` to mix the
UUIDv7 primary key into a custom base.
