...
:class:`~advanced_alchemy.types.GUID` stores UUIDs as ``RAW(16)`` on Oracle.
Values are generated by the application, so they are stable across inserts
and can be assigned deterministically by client code, ORM defaults, or
test fixtures.

For primary keys, use **UUIDv7** (time-ordered, RFC 9562). The
leading-timestamp bytes give monotonic insert order, which keeps Oracle's
B-tree indexes balanced.
