...
def visit_merge_clause(element: MergeClause, compiler: StrSQLCompiler, **kw: Any) -> str:
    ...
    if element.predicate is not None:
        case_predicate = f" AND {element.predicate._compiler_dispatch(compiler, **kw)!s}"  # pyright: ignore[reportPrivateUsage]  # noqa: SLF001
    ...
    if element.command == "INSERT":
        ...
        values = ", ".join(e._compiler_dispatch(compiler, **kw) for e in sets_tos)  # pyright: ignore[reportPrivateUsage] # noqa: SLF001
    ...
    if element.on_sets:
        values = ", ".join(
            f"{name} = {column._compiler_dispatch(compiler, **kw)}"  # pyright: ignore[reportPrivateUsage] # noqa: SLF001
...
def visit_merge(element: Merge, compiler: StrSQLCompiler, **kw: Any) -> str:
    clauses = " ".join(clause._compiler_dispatch(compiler, **kw) for clause in element.clauses)  # pyright: ignore[reportPrivateUsage]  # noqa: SLF001
