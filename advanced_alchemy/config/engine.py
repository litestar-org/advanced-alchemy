...
from advanced_alchemy.utils.dataclass import Empty, is_dataclass_instance
...
class PoolSettings:
    ...
    Controls pool capacity, timeouts, recycling, pre-ping, and LIFO behavior.
    """
...
@dataclass
class PoolInstance:
    """Connection pool class and instance configuration.

    Controls which pool class to use, an optional pre-built pool instance,
    pool logging name, and reset-on-return behavior.
    """
...
@dataclass
class LoggingConfig:
    """Logging configuration for SQLAlchemy's Engine.

    These options control how SQLAlchemy emits log output.
    """
...
@dataclass
class EngineBehaviorConfig:
    """Engine execution behavior configuration.

    Controls SQL execution, identifier limits, isolation, caching, and parameter style.
    """
...
@dataclass
class MiscConfig:
    """Miscellaneous engine configuration.

    These options cover connection arguments, DBAPI module overrides,
    execution options, and plugin loading.
    """
...
class EngineConfig:
    ...
    """Pool sizing and timeout settings. See :class:`PoolSettings` for details."""
    pool_instance: PoolInstance = field(default_factory=PoolInstance)
    """Pool class and instance settings. See :class:`PoolInstance` for details."""
    logging: LoggingConfig = field(default_factory=LoggingConfig)
    """Logging configuration. See :class:`LoggingConfig` for details."""
    behavior: EngineBehaviorConfig = field(default_factory=EngineBehaviorConfig)
    """Engine behavior configuration. See :class:`EngineBehaviorConfig` for details."""
    misc: MiscConfig = field(default_factory=MiscConfig)
    """Miscellaneous configuration. See :class:`MiscConfig` for details."""
    json_deserializer: "Callable[[str], Any]" = decode_json
    """For dialects that support the :class:`JSON <sqlalchemy.types.JSON>` datatype, this is a Python callable that will
    convert a JSON string to a Python object. By default, this is set to
    :attr:`decode_json() <.serialization.decode_json>`."""
    json_serializer: "Callable[[Any], str]" = encode_json
    """For dialects that support the JSON datatype, this is a Python callable that will render a given object as JSON.
    By default, :attr:`encode_json() <.serialization.encode_json>` is used."""

    def to_dict(self, exclude_empty: bool = True) -> "dict[str, Any]":
        """Convert the engine configuration to a flat dictionary.

        Sub-config dataclasses are flattened into the top-level dict so the
        result can be passed directly as ``**kwargs`` to ``create_engine()``.

        Args:
            exclude_empty: Whether to exclude fields set to the ``Empty`` sentinel.

        Returns:
            A flat dictionary of engine configuration key/value pairs.
        """
        result: dict[str, Any] = {}
        for fld in fields(self):
            ...
            if is_dataclass_instance(value):
                for sub_field in fields(value):
                    ...
                    if exclude_empty and sub_value is Empty:
                        continue
                    result[sub_field.name] = sub_value
            elif not (exclude_empty and value is Empty):
