...
class SQLAlchemyAsyncRepository(SQLAlchemyAsyncRepositoryProtocol[ModelT], FilterableRepository[ModelT]):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self._pk_columns: tuple[Column[Any], ...] = tuple(mapper.primary_key)  # type: ignore[union-attr]
        # Build mapping from ORM attribute names to PK column objects
        self._pk_attr_names: tuple[str, ...] = tuple(
            mapper.get_property_by_column(col).key  # type: ignore[union-attr]
            for col in self._pk_columns
        )
        ...
        *,
        ...
        with wrap_sqlalchemy_exception(
            ...
            if id_attribute is None:
                statement = statement.where(self._build_pk_filter(item_id))
            else:
                # Legacy path: custom id_attribute (single column lookup only)
                id_attr_name = id_attribute if not isinstance(id_attribute, InstrumentedAttribute) else id_attribute.key
                statement = self._filter_select_by_kwargs(statement, [(id_attr_name, item_id)])
