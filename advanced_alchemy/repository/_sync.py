    ...
    def _upsert_many_native(
        ...
        *,
        ...
        for chunk_start in range(0, len(input_dicts), rows_per_chunk):
            ...
            cursor_instances = self._execute_upsert_chunk(
                chunk=chunk,
                table=table,
                strategy=strategy,
        ...
        *,
        ...
        if strategy.kind == "on_conflict":
            ...
            if supports_returning:
                ...
                rows = self.session.scalars(returning_stmt)
                return list(rows)
        ...
        if strategy.kind == "merge":
            ...
            if isinstance(merge_result, list):
                for single in merge_result:
                    self.session.execute(single, additional_params or None)
            else:
                self.session.execute(merge_result, additional_params or None)
            return None

        if strategy.kind == "insert_or_update":
            self.session.execute(SpannerUpsert(table=table, values_list=chunk))
            return None

        msg = f"Unsupported upsert kind '{strategy.kind}' reached native execution"
        raise RepositoryError(msg)

    def _reselect_upsert_chunk(
        self,
        *,
        chunk: List[dict[str, Any]],
        match_fields: List[str],
